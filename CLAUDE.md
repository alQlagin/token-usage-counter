...
Both `*-log-summary.sh` scripts run essentially the same `jq` program: group messages by model, sum the four token fields, compute `cost_usd` per model from the pricing file, round the daily total to cents, and write `<source>-<session_id>_summary.json` to `<project>/.aiusage/YYYY-MM-DD/`. The output schema (including the `source: "claude"|"copilot"` discriminator, `transcript_path`, and `pricing_unit`) is **identical across runtimes by design** — downstream tooling reads files of either origin uniformly. If you change a field name, shape, or rounding rule, change it in *both* handlers and in the skill aggregator, or the daily report breaks.

The `transcript_path` field records the absolute path of the main transcript file that was read. For Claude Code this is the `.jsonl` under `~/.claude/projects/<hash>/`; for Copilot CLI it is the `events.jsonl` under `~/.copilot/session-state/<sid>/`. It is useful for tracing a summary back to its source transcript.

**Subagents differ by runtime.** Claude Code keeps each subagent's turns in a sibling `<transcript_path-without-.jsonl>/subagents/agent-*.jsonl` directory, *not* in the main transcript — so reading only `transcript_path` silently drops subagent tokens. The Claude handler globs that dir (when present) into the same `jq -s` so subagent usage folds into the one session summary. Model IDs are normalized through `def norm` (strips a trailing `-YYYYMMDD` release suffix) before grouping and pricing lookup, because subagents may report date-suffixed IDs (`claude-haiku-4-5-20251001`) that wouldn't match the bare pricing keys. **Copilot needs no equivalent:** verified against a real multi-agent session, Copilot writes subagent turns (e.g. `gpt-5-mini`) straight into the main `events.jsonl` as ordinary `assistant.message` events, so `group_by(.data.model)` already counts them. (Copilot does emit `subagent.started`/`subagent.completed` markers and an empty `research/` state dir, but the usage lives in the main transcript.)
