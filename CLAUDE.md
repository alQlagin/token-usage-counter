...
The Claude handler also slurps subagent transcripts. Claude Code keeps each subagent's turns in a sibling `<transcript_path-without-.jsonl>/subagents/agent-*.jsonl` directory, *not* in the main transcript — so reading only `transcript_path` silently drops subagent tokens. The handler globs that dir (when present) into the same `jq -s` so subagent usage folds into the one session summary. Model IDs are normalized through `def norm` (strips a trailing `-YYYYMMDD` release suffix) before grouping and pricing lookup, because subagents may report date-suffixed IDs (`claude-haiku-4-5-20251001`) that wouldn't match the bare pricing keys. **The Copilot handler does not yet do this** — Copilot's subagent transcript layout hasn't been verified against a real multi-agent session.
